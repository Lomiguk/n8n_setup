...
PostgreSQL намеренно не открыт в публичный интернет. На VPS host он привязан только к `127.0.0.1:5432` для администрирования через SSH tunnel, а контейнеры во внутренней Docker-сети могут подключаться с такими параметрами:
...
### Подключение из DBeaver или IDE

Порт PostgreSQL привязан к localhost на VPS, поэтому удаленные database tools должны подключаться через SSH. Так порт `5432` остается закрытым для публичного интернета, но администрирование с локальной рабочей машины остается доступным.

Создайте SSH tunnel с рабочей машины:

```bash
ssh -L 5432:127.0.0.1:5432 user@your-vps-host
```

Затем настройте DBeaver, DataGrip, VS Code или другой PostgreSQL client:

```text
Host:     127.0.0.1
Port:     5432
Database: значение POSTGRES_DEFAULT_DB, N8N_DB_NAME или другой базы данных
User:     значение POSTGRES_ADMIN_USER или отдельный database user
Password: соответствующий PostgreSQL password из .env
SSL:      disabled, если вы отдельно не добавили PostgreSQL TLS
```

В DBeaver можно либо держать terminal SSH tunnel открытым и использовать локальные настройки выше, либо указать те же SSH credentials VPS во вкладке SSH tunnel в DBeaver.
